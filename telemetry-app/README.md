...
``` shell
docker run --name postgres-db -e POSTGRES_DB=telemetry-db -e POSTGRES_USER=your_username -e POSTGRES_PASSWORD=your_password -p 5432:5432 -d postgres:13-alpine
```
...
spring:
  datasource:
    url: jdbc:postgresql://localhost:5432/telemetry-db
    username: user
    password: password
  jpa:
    hibernate:
